#include "CornerDBMaker.h"

using namespace std;

CornerDBMaker::CornerDBMaker(string _fileName)
    : fileName(_fileName), cornerDB(0xF) {}

CornerDBMaker::CornerDBMaker(string _fileName, uint8_t init_val)
    : fileName(_fileName), cornerDB(init_val) {}

bool CornerDBMaker::bfsAndStore() {
    RubiksCubeBitboard cube;
    queue<RubiksCubeBitboard> q;

    q.push(cube);
    cornerDB.setNumMoves(cube, 0);

    int curr_depth = 0;
    const int maxDepth = 6; // You can also set this dynamically

    while (!q.empty()) {
        int n = q.size();
        cout << "Exploring depth " << curr_depth << ", nodes: " << n << "\n";
        curr_depth++;
        if (curr_depth > maxDepth) break;

        for (int counter = 0; counter < n; counter++) {
            RubiksCubeBitboard node = q.front();
            q.pop();

            for (int i = 0; i < 18; i++) {
                auto curr_move = RubiksCube::MOVE(i);
                node.move(curr_move);

                if ((int)cornerDB.getNumMoves(node) > curr_depth) {
                    cornerDB.setNumMoves(node, curr_depth);
                    q.push(node);
                }

                node.invert(curr_move);
            }
        }
    }

    cout << "Writing to file: " << fileName << "\n";
    cornerDB.toFile(fileName);
    cout << "Done.\n";

    return true;
}
